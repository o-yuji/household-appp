      ...
      <Box component={"form"} onSubmit={handleSubmit(onSubmit)}>
        <Stack spacing={2}>
          ...
          <Controller
            ...
            control={control}
            render={({ field }) => (
              // <TextField {...field} id="カテゴリ" label="カテゴリ" select error={!!errors.category} helperText={errors.category?.message}>
              //   {categories.map((categories) => {
              //     return (
              //     <MenuItem value={categories.label} key={categories.label}>
              //       <ListItemIcon>
              //         {categories.icon}
              //       </ListItemIcon>
              //       {categories.label}
          ...
          {/* 保存ボタン */}
          {selectedTransaction && <Button onClick={handleDelete} variant="outlined" color="secondary" fullWidth>削除</Button>}
        </Stack>
      </Box>
    </Box>
  );
